...
export function testMbc(): boolean {
    ...
    describe("MBC1", () => {
        ...
        describe("ROM bank switching", () => {
            ...
            it("64-bank cart (SML2): write to $2000 does not crash", () => {
                // regression: assert(RomBankCount <= 32) fired for 64-bank carts
                setupMBCCart(CartridgeType.MBC1, 5, 0); // romSizeByte=5 → 64 banks
                mbcWrite(0x2000, 5);
                assertEquals<u8>(readRomBank1Sentinel(), 5, "bank 5 selected on 64-bank cart");
            });

            it("64-bank cart: low register max value (0x1F = 31) selectable", () => {
                setupMBCCart(CartridgeType.MBC1, 5, 0); // 64 banks
                mbcWrite(0x2000, 0x1F);
                assertEquals<u8>(readRomBank1Sentinel(), 31, "bank 31 (max 5-bit) selected");
            });

            it("64-bank cart: high+low registers combine to address upper banks", () => {
                // high=1, low=1 → rom1Bank = ((1<<5)|1) & 63 = 33
                setupMBCCart(CartridgeType.MBC1, 5, 0); // 64 banks
                mbcWrite(0x2000, 0x01); // low=1
                mbcWrite(0x4000, 0x01); // high=1
                assertEquals<u8>(readRomBank1Sentinel(), 33, "bank 33 = (1<<5)|1");
            });
